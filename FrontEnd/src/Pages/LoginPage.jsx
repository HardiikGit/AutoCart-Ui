      ...
      {formFields.map((field) => (
       <div key={field.id} style={{ marginBottom: '15px' }}>
        <div className="UserNameHeading" style={{ paddingBottom: '5px' }}>
         <p style={{ fontSize: '14px', color: 'var(--orange-color)' }}>{field.label}</p>
        </div>
        <div className="UserEmailPhn">
         <input
          type={field.type}
          placeholder={field.placeholder}
          style={{ width: '100%', padding: '10px 20px', border: '1px solid var(--orange-color)', borderRadius: '5px', color: 'var(--orange-color)', fontSize: '14px', fontWeight: '500' }}
          name={field.id}
          id={field.id}
         />
        </div>
     ...
     <div className="OtherLogin d-flex align-items-center justify-content-center" style={{ marginTop: '30px', gap: '30px' }}>
      {socialLinks.map((link) => (
       <div key={link.id} className="Social-links">
        <a href={link.url} style={{ fontSize: '22px', color: 'var(--orange-color)', transition: 'all ease .5s' }}>
         {link.icon}
        </a>
       </div>
      ))}
