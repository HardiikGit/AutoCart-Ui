...
function NextNavigationBar() {
   ...
   useEffect(() => {
      ...
      const CartRes = document.querySelector('.CartOpen')
      ...
      const handleScroll = () => {
         if (window.scrollY > 130) {
            ...
            CartRes.classList.add('Cart')
         }
         else {
            setIsSticky(false)
            CartOpenClone.classList.remove('Cart')
            CartRes.classList.remove('Cart')
         }
      }
               ...
               <div className="row align-items-center">
                  ...
                  <div className="col-lg-3">
                     <div className="WeeklyDeal d-flex justify-content-end" style={{ gap: '15px' }}>
                        <button className="CartOpen-1 relative" style={{ height: '32px', fontSize: '32px', fontWeight: '700', color: 'var(--white-color)', display: 'none', background: 'none', border: 'none' }} onClick={() => {
                           setShowCart(true)
                        }}>
                           <CiShoppingCart /><p style={{ position: 'absolute', top: '0', right: '0', fontSize: '14px', color: 'var(--white-color)', background: 'var(--orange-color)', paddingLeft: '3px' }}>{cart.length}</p>
                        </button>
